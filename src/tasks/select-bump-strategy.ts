...
import { updateNextVersion } from "#/tasks/create-context";
    ...
    return {
        recommended: (context: KiaraContext): ResultAsync<undefined, Error> =>
            getRecommendedVersion(context).map((version: string): undefined => {
                updateNextVersion(context, version);
                return undefined;
            }),
...
export function selectBumpStrategy(context: KiaraContext): ResultAsync<void, Error> {
    return context.options.bumpStrategy
        ...
        : promptStrategy().andThen((strategy) => executeStrategy(context, strategy as BumpStrategy));
}
